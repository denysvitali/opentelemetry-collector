...
func TestTraceID(t *testing.T) {
	tid := TraceID([]byte{1, 2, 3, 4, 5, 6, 7, 8, 8, 7, 6, 5, 4, 3, 2, 1})
	assert.Equal(t, []byte{1, 2, 3, 4, 5, 6, 7, 8, 8, 7, 6, 5, 4, 3, 2, 1}, []byte(tid))
	assert.False(t, tid.IsEmpty())
}

func TestNewTraceIDEmpty(t *testing.T) {
	tid := NewTraceIDEmpty()
	assert.Equal(t, []byte{}, []byte(tid))
...
func TestTraceIDImmutable(t *testing.T) {
	initialBytes := []byte{0x12, 0x34, 0x56, 0x78, 0x12, 0x34, 0x56, 0x78, 0x12, 0x34, 0x56, 0x78, 0x12, 0x34, 0x56, 0x78}
