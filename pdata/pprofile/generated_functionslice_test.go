...
func TestFunctionSlice(t *testing.T) {
	...
	es = newFunctionSlice(&otlpprofiles.Profile{}, &state)
	assert.Equal(t, 0, es.Len())
...
func TestFunctionSliceReadOnly(t *testing.T) {
	...
	es := newFunctionSlice(&otlpprofiles.Profile{}, &sharedState)
	assert.Equal(t, 0, es.Len())
...
func TestFunctionSlice_EnsureCapacity(t *testing.T) {
	...
	assert.Equal(t, es.Len(), cap(es.orig.GetFunctionTable()))
	...
	assert.Equal(t, ensureLargeLen, cap(es.orig.GetFunctionTable()))
	assert.Equal(t, generateTestFunctionSlice(), es)
}
...
func fillTestFunctionSlice(es FunctionSlice) {
	es.orig.SetFunctionTable(make([]*otlpprofiles.Function, 7))
	for i := 0; i < 7; i++ {
		es.orig.GetFunctionTable()[i] = &otlpprofiles.Function{}
		fillTestFunction(newFunction(es.orig.GetFunctionTable()[i], es.state))
	}
}
