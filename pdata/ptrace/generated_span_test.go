...
func TestSpan_ParentSpanID(t *testing.T) {
	ms := NewSpan()
	assert.Equal(t, pcommon.SpanID(data.SpanID([]byte{})), ms.ParentSpanID())
...
func fillTestSpan(tv Span) {
	...
	fillTestSpanEventSlice(newSpanEventSlice(utils.Ref(tv.orig.GetEvents()), tv.state))
	tv.orig.SetDroppedEventsCount(uint32(17))
	fillTestSpanLinkSlice(newSpanLinkSlice(utils.Ref(tv.orig.GetLinks()), tv.state))
